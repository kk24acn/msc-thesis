...
impl DkgService for MpcNodeService {
    ...
    async fn advance_dkg(
        ...
        let (output, is_done, keyshare_data, derivation_path) = {
            let mut engine_ref = self.dkg_sessions
                .get_mut(&key)
                .ok_or_else(|| Status::not_found("Session not found"))?;

            let (out, done) = engine_ref
                .advance_dkg(&req.payloads)
            ...
            if done {
                if let MpcSessionState::Dkg(DkgStateWrapper::Completed(ks)) = &engine_ref.state {
                    ks_data = Some(ks.clone());
                    path_data = engine_ref.derivation_path.clone();
                }
            }
            (out, done, ks_data, path_data)
        };

        if is_done {
            self.dkg_sessions.remove(&key);

            if let (Some(keyshare), Some(path_str)) = (keyshare_data, derivation_path) {
                ...
                return Err(Status::internal("Engine finished but missing keyshare or path"));
            }
        }
